...
class SingleDecisionCore:
    ...
    def filter_risk(self, features: dict, risk_state: dict) -> dict:
        ...
        if self.survivability is not None:
            ...
            if surv_check["reduce_risk"]:
                surv_factor = surv_check["recommended_size_factor"]
                size_multiplier *= surv_factor
                if surv_factor < 0.2:
                    return {
                        "risk_score": 0.9,
                        "size_multiplier": 0.0,
                        "verdict": "KILL",
                        "reason": f"SURVIVABILITY: {surv_check['reason']}",
                    }
    ...
    def compute_ev(self, features: dict, risk_filter: dict,
        ...
        # Use real slippage from LeanExecutor instead of hardcoded 0.0012.
        # Round-trip cost = 2 * one-way slippage + commission.
        ...
        avg_win -= estimated_cost
        avg_loss += estimated_cost

        # ── Noise discount ──
        entropy_discount = 1.0 - (noise * 0.5)

        # ── Base EV ──
        base_ev = (p_win * avg_win - (1 - p_win) * avg_loss) * entropy_discount
        ...
        if vol_ref > 0.04:
            ...
            dynamic_min_ev = self.min_ev_to_trade * 0.7  # TRENDING: 70% threshold
        elif vol_ref > 0.005:
            dynamic_min_ev = self.min_ev_to_trade * 0.3  # RANGING: 30% threshold
        else:
            dynamic_min_ev = self.min_ev_to_trade * 0.15  # LOW: 15% threshold
    ...
    def check_execution_feasibility(self, ev_result: dict,
        ...
        # ── Slippage check ──
        # If slippage > 50% of expected edge, not feasible
        ev_edge = abs(ev_result["adjusted_ev"])
        slippage_pct = slippage_bps / 10000.0
        if slippage_pct > ev_edge * 0.5 and ev_edge > 0:
            return {
                "feasible": False,
                "reason": f"slippage_too_high: {slippage_bps:.1f}bps > 50% of edge",
                "size_adjustment": 0.0,
            }
    ...
    def produce_action(self, features: dict, risk_filter: dict,
        ...
        # ── 3. Conviction too low → HOLD ──
        if features["conviction"] < self.min_confidence:
            return self._action_hold(
                f"low_conviction: {features['conviction']:.4f} < {self.min_confidence}"
            )
        ...
        # ── 8. COMPUTE SIZE (monotonic in risk) ──
        # base_size = conviction * kelly_fraction
        ...
        final_size = kelly_fraction * risk_size_mult * exec_adj * feedback_adj
